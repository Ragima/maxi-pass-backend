...
module VaultItem::Operation
  class Create < Trailblazer::Operation
    ...
    step :model!
    success :assign_vault!
    step Policy::Pundit(VaultItem::Policy::VaultItemPolicy, :create?)
    step Contract::Build(constant: VaultItem::Contract::Create)
    ...
    def model!(options, **)
      options['model.action'] = :new
      options[:model] = entity_class.new(options['contract.default.params'])
    end

    def assign_vault!(_options, model:, vault:, **)
      model.vault = vault
    end
    ...
    def encrypt_content_and_assign!(options, model:, current_user:, vault:, **)
      ...
      model.assign_attributes(content: encrypted_content)
    end
    ...
    def write_activity!(options, current_user:, model:, vault:, **)
      options[:activity] = ActivityService.new(current_user).call(
        key: "user.create_#{entity_sym}",
        action_type: entity_class.to_s.underscore.humanize,
        action_act: 'Create',
        actor_action: "created #{entity_sym.to_s.humanize.downcase}",
        subj1_id: model.id,
